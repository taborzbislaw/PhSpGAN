...
###########################################################
    ...
    photon_dim = params['x_dim'] - len(params['cond_keys'].split())
######################################################################
######################################################################
    ...
    while m < n:
        ...
        dum = Variable(torch.randn(batch_size, photon_dim)).type(dtypef)
######################################################################
######################################################################
...
class Generator_RoCGAN(nn.Module):
    '''
    Generator: G(z, θG) -> x fake samples
    ...
    def __init__(self, params, cmin, cmax):
        super(Generator_RoCGAN, self).__init__()
...
########################################################################################################
        ...
        self.map1_en = nn.Linear(z_dim, g_dim)
        self.map1_au = nn.Linear(self.x_dim, g_dim)

        self.maps_enc = nn.ModuleList()
        self.maps_au = nn.ModuleList()
        self.maps_dec = nn.ModuleList()

        for i in range(self.g_layers//2):
            self.maps_enc.append(nn.Linear(g_dim, g_dim))
            self.maps_au.append(nn.Linear(g_dim, g_dim))
            self.maps_dec.append(nn.Linear(g_dim, g_dim))
    ...
    def forward(self, x, y):
        ...
        #print('Generator in',x.shape)
        x = self.activ(self.map1_en(x))
        y = self.activ(self.map1_au(y))

        for i in range(self.g_layers//2):
            x = self.activ(self.maps_enc[i](x))
            y = self.activ(self.maps_au[i](y))

        dumx = x
        dumy = y

        for i in range(self.g_layers//2):
            x = self.activ(self.maps_dec[i](x))
            y = self.activ(self.maps_dec[i](y))
        ...
        y = torch.max(y, self.cmin)
        y = torch.min(y, self.cmax)

        #print('Generator out',x.shape)

        return x, y, dumx, dumy
...
#################################################################
        self.AUloss = torch.nn.MSELoss()
        self.LATloss = torch.nn.MSELoss()
#################################################################
        ...
        if ('start_pth' not in self.params) or (params['start_pth'] == None):
            ...
            self.G = Generator_RoCGAN(params, cmin, cmax)
        else:
            f = params['start_pth']
            print('Loading ', f)
...
########################################################################################################
                    ...
                    d_fake_data,_,_,_ = self.G(z,photonx)
                    d_fake_data = d_fake_data.detach()
...
######################################################################
                    ...
                    g_fake_data,reconstructed_data,lat_fake,lat_real = self.G(z,photonx)
...
######################################################################
                    ...
                    g_loss += self.params['au_weight'] * self.AUloss(photonx,reconstructed_data)
                    g_loss += self.params['lat_weight'] * self.LATloss(lat_fake,lat_real)
                ...
                if (epoch) % 500 == 0:
                    ...
                    print('Epoch %d d_loss: %.5f   g_loss: %.5f     d_real_loss: %.5f  d_fake_loss: %.5f'
                               %(epoch,
                                 ...
                                 d_fake_loss.data.item()),file=f)
                    f.close()

                    torch.save(self.G.state_dict(), 'Gen_' + self.params['model_name'])
                    torch.save(self.D.state_dict(), 'Dis_' + self.params['model_name'])

                    with open(self.params['params_name'], 'wb') as tf:
                        pickle.dump(self.params,tf)
